...
class TournamentInfoDTO {
  ...
  factory TournamentInfoDTO.fromJson(Map<String, dynamic> json) {
    return TournamentInfoDTO(
      ...
      startDateTime: DateTime.tryParse(json['startDateTime']?.toString() ?? '') ?? DateTime.fromMillisecondsSinceEpoch(0),
...
class PlayerHomePage extends StatefulWidget {
  ...
  static const String routeName = '/pyramid/feed/upcoming-tournaments';
...
class _PlayerHomePageState extends State<PlayerHomePage> {
  ...
  Future<void> _loadTournaments() async {
    ...
    try {
      // Construir URL a partir del baseUrl en AuthService (remueve '/auth' si procede)
      final root = AuthService.baseUrl.replaceFirst('/auth', '');
      final url = Uri.parse('$root/feed/upcoming-tournaments');
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pushNamed(context, '/pyramid/profile'); // ajusta si tu ruta es distinta
            },
